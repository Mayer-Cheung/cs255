...
void add_init_call(Function &F){
    if(F.getName().equals("main")){
        ...
        Constant *InitFn = M->getOrInsertFunction("init", Type::getVoidTy(F.getContext()), (Type *)0);
        CallInst::Create(InitFn,  "", &*I);


        Constant *Inc = M->getOrInsertFunction("increase", Type::getVoidTy(F.getContext()), (Type *)0);
        Constant *prt = M->getOrInsertFunction("print_stmt_cnt", Type::getVoidTy(F.getContext()), (Type *)0);

        for (Function::iterator BB = F.begin(), FE = F.end(); BB != FE; ++BB) {    
            for (BasicBlock::iterator II = BB->begin(), BBE = BB->end(); II != BBE; ++II) {    
                CallInst::Create(Inc, "", &*II);
            }

        }
        CallInst::Create(prt, "", &*I);
